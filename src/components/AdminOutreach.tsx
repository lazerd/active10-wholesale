...
export default function AdminOutreach() {
  ...
  const generate = async (p: Prospect) => { setBusy(p.id); setMsg(null); const d = await call({ action: "generate", prospectId: p.id, tone, length, instructions: instructions.trim() || undefined }); setBusy(null); if (d.ok) { setDrafts((x) => ({ ...x, [p.id]: d.touch })); if (d.source === "template") setMsg({ t: d.aiKey ? "⚠️ Gemini didn't respond — used a template (which ignores the style controls). Hit Regenerate to try AI again." : "⚠️ No Gemini key in production — using templates. Style controls only work with AI.", ok: false }); else setMsg({ t: "✨ AI-generated with your current style.", ok: true }); } else setMsg({ t: d.error || "Failed", ok: false }); };
