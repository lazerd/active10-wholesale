  ...
  if (isAdmin) {
    ...
    return (<div style={bg}>{fonts}{css}
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 24px 80px" }}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 24, background: "rgba(255,255,255,.03)", borderRadius: 12, padding: 4 }}>{["customers", "applicants", "orders", "affiliates", "winback", "restock", "abandoned"].map(t => <button key={t} onClick={() => { setAdminTab(t); setSelectedCustomer(null); setExpandedOrder(null); setEditingOrderId(null); }} style={{ flex: 1, padding: "10px 12px", borderRadius: 10, border: "none", background: adminTab === t ? `${B}33` : "transparent", color: adminTab === t ? "white" : "rgba(255,255,255,.5)", fontWeight: 600, fontSize: 12, cursor: "pointer", textTransform: "capitalize", transition: "all .2s" }}>{t === "winback" ? "Win-Back" : t === "abandoned" ? "Carts" : t}{t === "applicants" && pending.length > 0 ? ` (${pending.length})` : ""}</button>)}</div>
